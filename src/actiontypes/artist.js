export const ADD_ARTIST = 'artist/ADD_ARTIST';
export const REMOVE_ARTIST = 'artist/REMOVE_ARTIST';
export const UPDATE_ARTIST_RATING = 'artist/UPDATE_ARTIST_RATING';
// export const EDIT_ARTIST = 'artist/EDIT_ARTIST';
export const SELECT_ARTIST = 'artist/SELECT_ARTIST';
export const GO_TO_ARTIST = 'artist/GO_TO_ARTIST';
export const UPDATE_ARTIST = 'artist/UPDATE_ARTIST';
